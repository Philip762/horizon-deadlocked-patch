      ...
      if (upgradeMoby) {
        if (!playerData->ActionCooldownTicks) {
          ...
          if (vector_sqrmag(t) < (UPGRADE_PICKUP_RADIUS * UPGRADE_PICKUP_RADIUS)) {
            ...
            if (playerData->State.Upgrades[i] < upgradeDef.Max) {
...
void spawnUpgrades(void)
{
  ...
  for (i = 0; i < mapConfig->UpgradeDefCount; ++i) {
    ...
    if (mapConfig && mapConfig->CreateUpgradePickupFunc) {
      DPRINTF("SPAWNING UPGRADE WITH ID %d AND MAX OF %d (def count %d, spawn idx %d)\n", upgradeId, &mapConfig->UpgradeDefs[i].Max, mapConfig->UpgradeDefCount, bakedSpIdx);
  ...
  if (!State.GameOver)
  {
    ...
    else
    {
      ...
      if (State.IsHost && (gameTime - State.RoundStartTime) > ROUND_START_DELAY_MS)
      {
        ...
        int canSpawn = 1;
        if (mapConfig && mapConfig->CanSpawnMobsFunc)
          canSpawn = mapConfig->CanSpawnMobsFunc();
